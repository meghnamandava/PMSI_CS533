...
int main(int argc, char *argv[])
{
    ...
    if( global == 0 )
		...
		unsigned long long endAddr = (unsigned long long)global + sizeof(Global);
    ...
    for (i = 0; i < n_processors; i++)
		...
		startAddr = (uint64_t)&timing[0][0];
		endAddr = (uint64_t)&timing[n_processors-1][n_processors-1] + sizeof(Timing);
