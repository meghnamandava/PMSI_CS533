...
/* initialize constants and variables
   ...
   if (do_output) {
	 ...
	 unsigned long long startAddr = (unsigned long long)multi;
	 unsigned long long endAddr = (unsigned long long)multi + sizeof(struct multi_struct);	 
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)global;
	 endAddr = (unsigned long long)global + sizeof(struct global_struct);	 
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)bars;
	 endAddr = (unsigned long long)bars + sizeof(struct bars_struct); 
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)locks;
	 endAddr = (unsigned long long)locks + sizeof(struct locks_struct);
	 m5_addsymbol(startAddr, endAddr);
