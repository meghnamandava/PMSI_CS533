...
/*
	 ...
	 startAddr = (uint64_t)&key[0][0];
	 endAddr = (uint64_t)&key[0][num_keys-1];

	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (uint64_t)&key[1][0];
	 endAddr = (uint64_t)&key[1][num_keys-1];

	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (uint64_t)&key_partition[0];
	 endAddr = (uint64_t)&key_partition[number_of_processors];
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (uint64_t)&rank_partition[0];
	 endAddr = (uint64_t)&rank_partition[number_of_processors];
	 m5_addsymbol(startAddr, endAddr);
	 */
