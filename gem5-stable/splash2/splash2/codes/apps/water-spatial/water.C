...
int main(int argc, char **argv)
{
    ...
    /* read input */
		...
		TSTEP=1.5e-16;
		NMOL=64;
		NSTEP=3; NORDER=6; NSAVE=-1; NRST=3000; NPRINT=3; NFMC=0; NumProcs=4; CUTOFF=6.212752;
/*
	...
	*/
    ...
    if (NSAVE > 0) {  /* not true for input decks provided */
		...
		unsigned loopCnt1 = 0, loopCnt2 = 0, loopCnt3 = 0;
		for (loopCnt1 = 0; loopCnt1 < BOX_PER_SIDE; loopCnt1++) {
			for (loopCnt2 = 0; loopCnt2 < BOX_PER_SIDE; loopCnt2++) {
				for (loopCnt3 = 0; loopCnt3 < BOX_PER_SIDE; loopCnt3++) {
					unsigned long long startAddr = (unsigned long long)&BOX[loopCnt1][loopCnt2][loopCnt3].list->mol;
					unsigned long long endAddr = (unsigned long long)&BOX[loopCnt1][loopCnt2][loopCnt3].list->mol + sizeof(molecule_type);
