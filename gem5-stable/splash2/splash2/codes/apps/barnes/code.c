...
string defv[] = {                 /* DEFAULT PARAMETER VALUES              */
    ...
    "nbody=4096",                /* number of particles to generate       */
...
#line 268
   ...
   printf("COMPUTESTART  = %12lu\n",Global->computestart);
	 ...
	 unsigned long long startAddr = (unsigned long long)&Local[0];
	 unsigned long long endAddr = (unsigned long long)&Local[MAX_PROC-1];
...
void tab_init()
{
   ...
   Local[0].mybodytab = (bodyptr*) malloc(NPROC*maxmybody*sizeof(bodyptr));;
	 ...
	 unsigned long startAddr = (unsigned long long)&Local[0].mybodytab[0];
	 unsigned long endAddr = (unsigned long long)&Local[0].mybodytab[NPROC*maxmybody-1];

	 //printf("\n QS and E %lx %lx", startAddr, endAddr);
	 //m5_addsymbol(startAddr, endAddr);
   ...
   Local[0].myleaftab = (leafptr*) malloc(NPROC*maxmyleaf*sizeof(leafptr));;
	 ...
	 startAddr = (unsigned long long)&Local[0].mycelltab[0];
	 endAddr = (unsigned long long)&Local[0].mycelltab[NPROC*maxmycell-1];

	 printf("\n RS and E %lx %lx", startAddr, endAddr);
	 //m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&Local[0].myleaftab[0];
	 endAddr = (unsigned long long)&Local[0].myleaftab[NPROC*maxmyleaf-1];

	 printf("\n PS and E %lx %lx", startAddr, endAddr);
	 //m5_addsymbol(startAddr, endAddr);
