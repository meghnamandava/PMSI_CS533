...
int main(int argc, char *argv[])
{
  ...
  if (proc_bytes == NULL) {
	...
	//uint64_t startAddr = (uint64_t)&proc_bytes[0];
	//uint64_t endAddr = (uint64_t)&proc_bytes[P-1];
...
/* Note that this causes all blocks to start out page-aligned, and that
  ...
  for (i=0;i<nblocks;i++) {
	...
	//printf("\n address ranges: %lx %lx %lx %lx", a, &a[nblocks*nblocks-1], a[0], a[nblocks*nblocks-1]);
  ...
  Global = (struct GlobalMemory *) G_MALLOC(sizeof(struct GlobalMemory));
	...
	unsigned long long endAddr = (unsigned long long)Global + sizeof(struct GlobalMemory);
	m5_addsymbol(startAddr,endAddr);
  
	Global->t_in_fac = (double *) G_MALLOC(P*sizeof(double));
	startAddr = (unsigned long long)&Global->t_in_fac[0];
	endAddr = (unsigned long long)&Global->t_in_fac[P-1];
	m5_addsymbol(startAddr,endAddr);

  Global->t_in_mod = (double *) G_MALLOC(P*sizeof(double));
	startAddr = (unsigned long long)&Global->t_in_mod[0];
	endAddr = (unsigned long long)&Global->t_in_mod[P-1];
	m5_addsymbol(startAddr,endAddr);

  Global->t_in_solve = (double *) G_MALLOC(P*sizeof(double));
	startAddr = (unsigned long long)&Global->t_in_solve[0];
	endAddr = (unsigned long long)&Global->t_in_solve[P-1];
	m5_addsymbol(startAddr,endAddr);

  Global->t_in_bar = (double *) G_MALLOC(P*sizeof(double));
  startAddr = (unsigned long long)&Global->t_in_bar[0];
	endAddr = (unsigned long long)&Global->t_in_bar[P-1];
	m5_addsymbol(startAddr,endAddr);
	
	Global->completion = (double *) G_MALLOC(P*sizeof(double));
	startAddr = (unsigned long long)&Global->completion[0];
	endAddr = (unsigned long long)&Global->completion[P-1];
	m5_addsymbol(startAddr,endAddr);
...
*/
  ...
  if (doprint) {
	...
	startAddr = (unsigned long long)a;
	endAddr = (unsigned long long)&a[nblocks*nblocks-1];	
	m5_addsymbol(startAddr, endAddr);

	//startAddr = (uint64_t)a[0];
	//endAddr = (uint64_t)a[nblocks*nblocks-1];	
