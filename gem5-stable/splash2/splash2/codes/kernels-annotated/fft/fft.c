...
#line 134
  ...
  umain2 = (double *) malloc(2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE);;
	...
	startAddr = (unsigned long long)x;
	endAddr = (unsigned long long)x + (2*N+rootN*pad_length)*sizeof(double)+PAGE_SIZE;
	m5_addsymbol(startAddr, endAddr);

	startAddr = (unsigned long long)trans;
	endAddr = (unsigned long long)trans + 2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE;
	m5_addsymbol(startAddr, endAddr);
	
	startAddr = (unsigned long long)umain;
	endAddr = (unsigned long long)umain + 2*rootN*sizeof(double);
	m5_addsymbol(startAddr, endAddr);

	startAddr = (unsigned long long)umain2;
	endAddr = (unsigned long long)umain2 + 2*(N+rootN*pad_length)*sizeof(double)+PAGE_SIZE;
	m5_addsymbol(startAddr, endAddr);
