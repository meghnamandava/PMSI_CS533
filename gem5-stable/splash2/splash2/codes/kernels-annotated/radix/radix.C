#line 185 "/home/n9sritha/Desktop/splash2/codes/null_macros/c.m4.null.POSIX_BARRIER"
...
#line 135
   ...
   if ((key[0] == NULL) || (key[1] == NULL) || (key_partition == NULL) || (rank_partition == NULL) || 
	 ...
	 startAddr = (unsigned long long)&key[0][0];
	 endAddr = (unsigned long long)&key[0][num_keys-1];

	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&key[1][0];
	 endAddr = (unsigned long long)&key[1][num_keys-1];

	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&key_partition[0];
	 endAddr = (unsigned long long)&key_partition[number_of_processors];
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&rank_partition[0];
	 endAddr = (unsigned long long)&rank_partition[number_of_processors];
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)global;
	 endAddr = (unsigned long long)global + sizeof(struct global_memory);
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&global->ranktime[0];
	 endAddr = (unsigned long long)&global->ranktime[number_of_processors-1];
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&global->sorttime[0];
	 endAddr = (unsigned long long)&global->sorttime[number_of_processors-1];
	 m5_addsymbol(startAddr, endAddr);

	 startAddr = (unsigned long long)&global->totaltime[0];
	 endAddr = (unsigned long long)&global->totaltime[number_of_processors-1];
	 m5_addsymbol(startAddr, endAddr);
   ...
   for (i=0;i<number_of_processors;i++) {
     *temp = (long *) a;
	 ...
	 startAddr = (uint64_t)rank_me;
	 endAddr = (uint64_t)rank_me + size*sizeof(long);
	 m5_addsymbol(startAddr, endAddr);
